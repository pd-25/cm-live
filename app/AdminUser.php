...
class AdminUser extends Model
{
     ...
     * @var array
     */
	protected $fillable = array('username','password', 'new_password','role_id','company_id','first_name','last_name','client_ip');
	...
	*@return  count
    */
	public function checkEmailDuplicacyWithType($email,$typ)
	{
		$email = strtoupper(trim($email));
		$hotel_user_data=AdminUser::where(DB::raw('upper(username)'),$email)->where('user_type',$typ)->where('is_trash' ,0)->first();
		if($hotel_user_data)
		{
