...
use App\CmOtaBooking;//used for writing the data in the CmOtaBooking
use App\CmOtaBookingRead;//uesd for reading the data from CmOtaBooking
...
class OtaBookingController extends Controller
{
    ...
    $query=  CmOtaBookingRead::select(DB::raw('distinct ota_id,hotel_id,unique_id,customer_details,booking_status,channel_name,payment_status,inclusion,no_of_adult,no_of_child,rooms_qty,room_type,checkin_at,tax_amount,checkout_at,booking_date,rate_code,amount,currency,payment_status,confirm_status,cancel_status,ip,ids_re_id'))
      ...
      $data=$query->orderBy('unique_id','DESC')->paginate(100);
