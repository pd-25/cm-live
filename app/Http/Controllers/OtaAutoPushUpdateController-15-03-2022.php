...
public function handleWinhms($bucket_data,$booking_data)
{
	$bucket_booking_status=$bucket_data['bucket_booking_status'];
	$hotel_id = $booking_data['booking_hotel_id'];
	$ids_status=$this->winhmscontroller->getWinhmsStatus($hotel_id);
	$customer_data=array();
    if($ids_status)
    {
		$ids_data=$this->prepare_ids_data($hotel_id,$booking_data);
		...
		$type=$booking_data['booking_channel'];
		$ids_string="";
		$last_ids_id=$this->winhmscontroller->winhmsBookings($hotel_id,$type,$ids_data,$customer_data,$bucket_booking_status);
		if($last_ids_id)
        {
			DB::table('cm_ota_booking')
            ->where('id',$bucket_data['bucket_ota_booking_tabel_id'])
			->update(['ids_re_id' => $last_ids_id]);
			$ids_string=$this->winhmscontroller->getWinhmsString($last_ids_id);
			if($this->winhmscontroller->pushReservations($ids_string,$last_ids_id)){
				DB::table('winhms_reservation')->where('id', $last_ids_id)
								->update(['ids_confirm' => 1]);
			}else{
				DB::table('winhms_reservation')->where('id', $last_ids_id)
								->update(['ids_confirm' => 2]);
