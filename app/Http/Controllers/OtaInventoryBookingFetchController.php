...
class OtaInventoryBookingFetchController extends Controller
{
    ...
    public function getRatePlanByRoomType(int $ota_id,int $hotel_id,string $date_from,string $date_to,$room_type_id){
        $date_from=date('Y-m-d',strtotime($date_from));
        $date_to=date('Y-m-d',strtotime($date_to));
        $room_type_n_rate_plans=DB::table('kernel.room_rate_plan')
            ...
            ->get();
        $get_ota_name = CmOtaDetails::select('ota_name')->where('ota_id',$ota_id)->first();
        $ota_name = $get_ota_name->ota_name;
        ...
        if($room_type_n_rate_plans){
            foreach($room_type_n_rate_plans as $all_types){
                if($all_types->rate_plan_id){
                    $data=$this->getRatesByRoomnRatePlan($ota_name, $date_from, $date_to, $all_types->room_type_id,$all_types->rate_plan_id,$hotel_id,$hex_code,$currency);
    ...
    public function getRatePlan(int $ota_id,int $hotel_id,string $date_from,string $date_to){
        ...
        $get_ota_name = CmOtaDetails::select('ota_name')->where('ota_id',$ota_id)->first();
        $ota_name = (isset($get_ota_name->ota_name)) ? $get_ota_name->ota_name : '';
        $hotel_info=DB::table('kernel.hotels_table')->select('company_id')->where('hotel_id',$hotel_id)->first();
        $comp_info=DB::table('kernel.company_table')->select('hex_code','currency')->where('company_id',$hotel_info->company_id)->first();
        $hex_code=(isset($comp_info->hex_code)) ? $comp_info->hex_code : '';
        $currency=(isset($comp_info->currency)) ? $comp_info->currency : '';
        if($room_type_n_rate_plans){
            foreach($room_type_n_rate_plans as $all_types){
                if($all_types->rate_plan_id){
                    $room_type_id = (isset($all_types->room_type_id)) ? $all_types->room_type_id : 0;
                    $data=$this->getRatesByRoomnRatePlan($ota_name, $date_from, $date_to, $room_type_id,$all_types->rate_plan_id,$hotel_id,$hex_code,$currency);
                    $all_types->rates=$data;
                }
                else{
                    unset($all_types);
    ...
    public function getRatesByRoomnRatePlan($ota_name,string $date_from ,string $date_to,int $room_type_id,int $rate_plan_id,$hotel_id,$hex_code,$currency)
    {
        $date1=date_create($date_from);
        $date2=date_create($date_to);
        $date3=date_create(date('Y-m-d'));
        ...
        for($i=1;$i<=$diff; $i++)
        {
            ...
            else
            {
                ...
                $block_status = (isset($rate_plan_log_details->block_status)) ? $rate_plan_log_details->block_status : '';
                if($multiple_days!=null)
                {
                    if($multiple_days->$day==0)
                    {
