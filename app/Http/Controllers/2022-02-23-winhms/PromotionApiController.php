        ...
        if($status == 'creation'){
            ...
            $promotion_xml = '<?xml version="1.0" encoding="UTF-8"?>
                    ...
                    if($blackout){
                        ...
                        for($i=0;$i<sizeof($black_out_dates);$i+2){
                            $promotion_xml .= '<DateRange Start="'.$black_out_dates[$i].'" End="'.$black_out_dates[$i+1].'"/>';
                        }
                        $promotion_xml .= '</BlackoutDateRange>';
                    }
                    $promotion_xml.='<CancellationPolicy Id="3434"/>
                                        <Discount>
                    ...
                    // </CustomerSegments>
                $curl_call = $this->curlCall($url,$headers,$promotion_xml);
                $curl_call_array = json_decode(json_encode(simplexml_load_string($this->removeNamespaceFromXML($response))), true);
            ...
            if($status == 'updation'){
                ...
                $xml_header = 'HotelPromotionToggle_RQ';
