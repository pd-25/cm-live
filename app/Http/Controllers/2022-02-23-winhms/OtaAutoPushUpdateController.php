...
class OtaAutoPushUpdateController extends Controller{
	...
	public function bookingjiniUpdate($bucket_data,$booking_data){
		...
		if(1){
			...
			if($is_winhms){
				$this->handleWinhms($bucket_data,$booking_data,$booking_data['booking_channel']);
			}
		}
		$booking_ip='1.1.1.1';
...
public function handleWinhms($bucket_data,$booking_data) //change according to live code.
{
	$bucket_booking_status=$bucket_data['bucket_booking_status'];
	$hotel_id = $booking_data['booking_hotel_id'];
    ...
    if($winhms_status)
    {
		...
		if($bucket_booking_status == 'Cancel'){
			$last_winhms_id = $this->winhmscontroller->winhmsCancelBooking($hotel_id,$type,$winhms_data,$customer_data);
		}else{
...
public function prepare_winhms_data($hotel_id,$ota_booking_data)// change according to live code.
{
	...
	$no_of_room_types=sizeof($room_types);
    	foreach($room_types as $key=>$room_type)
        {
            ...
            $arr=array('room_type_id'=>$room_type,'rate_plan_id'=>$hotel_rate_plan_id,'adults'=>$total_adult,'from_date'=>$from_date,'to_date'=>$to_date,'rates'=>$rates_arr,'no_of_rooms'=>1);
			array_push($booking_data['room_stay'],$arr);
			}
		}
    return $booking_data;
