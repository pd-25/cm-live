...
use App\Models\tours\RoomRatePlan;
// use App\Models\Coupon;
use App\Models\tours\RoomTypeTable;
 ...
 * function getInvByHotel for fetching data
    ...
    * function getInvByHotel for fetching data
    ...
    public function bookings(string $api_key, Request $request)
    {
        ...
        $booking_Details = json_decode($request->getContent());
        $validData = $this->validateBookingsData($booking_Details, $diff);
        if ($validData['status'] == 1) {
            ...
            if ($validCart) {
                ...
                if ($payment_status->partial_payment == 1) {
                    ...
                    } elseif ($percentage == $payment_status->partial_pay_amt) {
                ...
                if ($offeringMethod == 'OPT-IN') {
                    $rp_member_id = 295;
                } elseif ($offeringMethod == 'OPT-OUT') {
                    $rp_member_id = 298;
                }
                if ($source  != 'GEMS') {
                    $source = '';
                }
                $inv_data['invoice']=$this->createInvoice($hotel_id, $cart, $coupon, $paid_service, $from_date, $to_date, $inv_data['user_id'], $percentage, $inv_data['ref_no'], $refund_protect_price_info, $rp_member_id, $source);
            ...
            return $validData;
            // $res = array('status' => 1, 'message' => $validData['message']);
            // return response()->json($res);
        }
    }

    public function validateBookingsData($booking_Details, $diff)
    {
        $failure_message='Tour Booking failed due to unsuffcient booking details';
            ...
            if ($from_date <= $to_date && $today <= $from_date) {
                if ($carts) {
                    ...
                    foreach ($carts as $key=>$cart) {
                        ...
                        foreach ($rooms as $room) {
                            ...
                            foreach ($day_wise_rates as $day_wise_rate) {
                                ...
                                $db_rate_plan = RoomRatePlan::where('hotel_id', $hotel_id)
                                ...
                                if ($db_rate_plan) {
                                    ...
                                    if ($coupon) {
                                        if (isset($coupon[$key]->applied_coupon)) {
                                            foreach ($coupon as $cop) {
                                                if (isset($cop->applied_coupon)) {
                                                    $appliedCoupon = $cop->applied_coupon;
                                                    if ($appliedCoupon->room_type_id==$room_type_id) {
                                                        $couponId = $appliedCoupon->coupon_id;
                                                        $couponDetails = Coupons::where('coupon_id', $couponId)->where('hotel_id', $hotel_id)->first();
                    ...
                    if (round($db_total_price_paid, 0) == round($paid_amount, 0) && round($db_gst_price, 0) == round($gst_price_after_coupon, 0)) {
                        ...
                        $res=array('status'=>0,"message"=>'Invalid 5');
                        return $res;
                    }
                } else {
                    $res=array('status'=>0,"message"=>'Invalid 7');
                    return $res;
                }
            } else {
                $res=array('status'=>0,"message"=>'Invalid 9');
                return $res;
            }
        } else {
            ...
            if (isset($coupon)) {
                foreach ($coupon as $key1 => $coup) {
                    if (sizeof($coupon) == 1) {
                        ...
                        } elseif ($coup['room_data']['room_type_id'] == 0) {
                            $coupon_info = $coupon[$key1];
                        } else {
                        }
                    }
    ...
    $diff1=$diff1->format("%a");
        ...
        foreach ($rooms as $room) {
            ...
            } elseif ($selected_adult==$max_adult) {
                $extra_adult_ok=true;
            } else {
            ...
            } elseif ($selected_child==$max_child) {
                $extra_child_ok=true;
            } else {
                $extra_child_ok=true;
            }
        ...
        } elseif ($baseCurrency!='INR') {  //Other tax module check
    ...
    **/
    ...
    public function handleWimhms($cart, $from_date, $to_date, $booking_date, $hotel_id, $user_id)
    {
        $booking_status='Commit';
        ...
        if ($winhms_status) {
            ...
            $customer_data=$this->getUserDetails($user_id)->toArray();
            $type="Bookingjini";
            $last_winhms_id=$this->WinhmsController->winhmsBookings($hotel_id, $type, $winhms_data, $customer_data, $booking_status);
            if ($last_winhms_id) {
  ...
  ->get();
    ...
    public function getGstPricePerRoom($room_rate, $discount)
    {
        ...
        if ($room_rate >=0 && $room_rate<1000) {
            $gstPrice = 0;
        } elseif ($room_rate >=1000 && $room_rate<7500) {
