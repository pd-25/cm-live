...
class BookingjiniPMSController extends Controller
{
    ...
    */
    ...
    public function bookingjiniRestriction(Request $request)
    {
        ...
        if(!$postdata)
        {
            $return_array = '<?xml version="1.0" encoding="utf-8"?><OTA_HotelRestrictionInfoRS
    xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance" xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema"
    xmlns="http://www.bookingjini.com/OTA/2017/01" EchoToken="'.$token.'" TimeStamp="'.$timestamp.'"
    Version="'.$version.'">
    <Errors xmlns="">
...
if(($rest_status == 'Close' && $on_restrict == 'Arrival') || ($rest_status == 'Close' && $on_restrict == 'Departure') ||
($rest_status == 'Close' && $on_restrict == 'Master')){
$getOtaNames = array();
...
$get_number_of_room = OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','<=',$start_date)->
    where('date_to','>=',$end_date)->first();
    ...
    $get_number_of_room = OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','<=',$start_date)->
        ...
        $get_number_of_room = Inventory::select('no_of_rooms')->where($condition)->where('date_from','<=',$start_date)->
            where('date_to','>=',$end_date)->first();
            if(sizeof($get_number_of_room)>0){
            ...
            <OTA_HotelRestrictionInfoRS xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance"
                xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema" xmlns="http://www.bookingjini.com/OTA/2017/01"
            ...
            $ota_name=strtolower($ota_name);
            $condition=array('block_status'=>0,'room_type_id'=>$room_type_id,'hotel_id'=>$hotel_id,'channel'=>$ota_name);
            $get_number_of_room = OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','
            <=',$start_date)->where('date_to','>=',$end_date)->first();
                if(sizeof($get_number_of_room)>0){
                ...
                <=',$start_date)->where('date_to','>=',$end_date)->first();
                    ...
                    $get_number_of_room = Inventory::select('no_of_rooms')->where($condition)->where('date_from','
                    <=',$start_date)->where('date_to','>=',$end_date)->first();
                        if(sizeof($get_number_of_room)>0){
                        ...
                        <OTA_HotelRestrictionInfoRS xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance"
                            xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema"
                            xmlns="http://www.bookingjini.com/OTA/2017/01" EchoToken="'.$token.'"
                        ...
                        $condition=array('block_status'=>0,'room_type_id'=>$room_type_id,'hotel_id'=>$hotel_id,'channel'=>$ota_name);
                        $get_number_of_room =
                        OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','<=',$start_date)->
                            where('date_to','>=',$end_date)->first();
                            if(sizeof($get_number_of_room)>0){
                            ...
                            $client_ip=$this->ipAddress->getIPAddress();
                            $getOtaDetails =
                            CmOtaDetails::select('ota_id','ota_name')->where('hotel_id',$hotel_id)->get();
                            if(sizeof($getOtaDetails) > 0){
                            $i=0;
                            foreach($getOtaDetails as $otas){
                            ...
                            $condition=array('block_status'=>0,'room_type_id'=>$room_type_id,'hotel_id'=>$hotel_id,'channel'=>$ota_name);
                            $get_number_of_room =
                            OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','
                            <=',$start_date)->where('date_to','>=',$end_date)->first();
                                if(sizeof($get_number_of_room)>0){
                                ...
                                $get_number_of_room =
                                Inventory::select('no_of_rooms')->where($condition)->where('date_from','
                                <=',$start_date)->where('date_to','>=',$end_date)->first();
                                    if(sizeof($get_number_of_room)>0){
                                    ...
                                    if(($rest_status == 'Close' && $on_restrict == 'Arrival') || ($rest_status ==
                                    'Close' && $on_restrict == 'Departure') || ($rest_status == 'Close' && $on_restrict
                                    == 'Master')){
                                    $getOtaNames = array();
                                    if($on_restrict == 'Arrival'){
                                    $restriction_status = 'CTA';
                                    ...
                                    $inv_type_code =
                                    $available_status["StatusApplicationControl"]['@attributes']['InvTypeCode'];
                                    $rateplan_code =
                                    $available_status["StatusApplicationControl"]['@attributes']['RatePlanCode'];
                                    $meal_plan_code =
                                    $available_status["StatusApplicationControl"]['@attributes']['MealPlanCode'];
                                    ...
                                    $multiple_days['Sun'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Sun"];
                                    $multiple_days['Mon'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Mon"];
                                    $multiple_days['Tue'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Tue"];
                                    $multiple_days['Wed'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Weds"];
                                    $multiple_days['Thu'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Thur"];
                                    $multiple_days['Fri'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Fri"];
                                    $multiple_days['Sat'] =
                                    $available_status["StatusApplicationControl"]['@attributes']["Sat"];

                                    if(isset($available_status["StatusApplicationControl"]['DestinationSystemCodes'])){
                                    $isExist = array();
                                    foreach ($available_status["StatusApplicationControl"]['DestinationSystemCodes'] as
                                    $value) {
                                    if(is_array($value)){
                                    foreach($value as $val){
                                    $getOtaName=DB::table('cm_ota_credential_parameter')->select('ota_name')->where('id',$val)->first();
                                    ...
                                    <=',$start_date)->where('date_to','>=',$end_date)->first();
                                        ...
                                        $available_status["StatusApplicationControl"]['@attributes']['MealPlanCode'];
                                        $start_date =
                                        $available_status["StatusApplicationControl"]['@attributes']['Start'];
                                        $end_date = $available_status["StatusApplicationControl"]['@attributes']['End'];
                                        $multiple_days['Sun'] =
                                        $available_status["StatusApplicationControl"]['@attributes']["Sun"];
                                        $multiple_days['Mon'] =
                                        ...
                                        foreach ($available_status["StatusApplicationControl"]['DestinationSystemCodes']
                                        as $value) {
                                        if(is_array($value)){
                                        foreach($value as $val){
                                        $getOtaName=DB::table('cm_ota_credential_parameter')->select('ota_name')->where('id',$val)->first();
                                            ...
                                            <=',$start_date)->where('date_to','>=',$end_date)->first();
                                                ...
                                                <=',$start_date)->where('date_to','>=',$end_date)->first();
                                                    ...
                                                    <OTA_HotelRestrictionInfoRS
                                                        ...
                                                        xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema"
                                                        xmlns="http://www.bookingjini.com/OTA/2017/01"
                                                        EchoToken="'.$token.'" TimeStamp="'.$timestamp.'"
                                                        Version="'.$version.'">
                                                        <Success />
                                                    </OTA_HotelRestrictionInfoRS>';
                                                    return response($return_array)->header('Content-Type',
                                                    'application/xml');
                                                    }
                                                    else
                                                    {
                                                    ...
                                                    $los =$available_status
                                                    ["LengthsOfStay"]["LengthOfStay"]["@attributes"]["Time"];
                                                    ...
                                                    $available_status["StatusApplicationControl"]['@attributes']['Start'];
                                                    $end_date =
                                                    $available_status["StatusApplicationControl"]['@attributes']['End'];
                                                    $multiple_days['Sun'] =
                                                    $available_status["StatusApplicationControl"]['@attributes']["Sun"];
                                                    $multiple_days['Mon'] =
                                                    $available_status["StatusApplicationControl"]['@attributes']["Mon"];
                                                    ...
                                                    foreach
                                                    ($available_status["StatusApplicationControl"]['DestinationSystemCodes']
                                                    as $value) {
                                                    if(is_array($value)){
                                                    foreach($value as $val){
                                                    $getOtaName=DB::table('cm_ota_credential_parameter')->select('ota_name')->where('id',$val)->first();
                                                    ...
                                                    {
                                                    $room_type_id =
                                                    $this->pmsService->pmsRoom($hotel_id,$inv_type_code);
                                                    $rate_plan_id =
                                                    $this->pmsService->pmsRate($room_type_id,$meal_plan_code);
                                                    if(sizeof($getOtaNames) > 0){
                                                    $i=0;
                                                    foreach($getOtaNames as $ota_name){
                                                    $ota_name=strtolower($ota_name);
                                                    $condition=array('block_status'=>0,'room_type_id'=>$room_type_id,'hotel_id'=>$hotel_id,'channel'=>$ota_name);
                                                    $get_number_of_room =
                                                    OtaInventory::select('no_of_rooms')->where($condition)->where('date_from','>=',$start_date)->where('date_to','
                                                    <=',$end_date)->first();
                                                        if(sizeof($get_number_of_room)>0){
                                                        $data=['date_from'=>$start_date,'date_to'=>$end_date,'room_type_id'=>$room_type_id,'user_id'=>0,'ota_id'=>0,'hotel_id'=>$hotel_id,'no_of_rooms'=>$get_number_of_room->no_of_rooms,'pms_id'=>$pms_id,'pms_name'=>$pms_name,'client_ip'=>$ip,'los'=>$los,'ota_details'=>$ota_name];
                                                        $user_id=$data['user_id'];
                                                        ...
                                                        <OTA_HotelRestrictionInfoRS
                                                            ...
                                                            Version="'.$version.'">
                                                            <Errors xmlns="">
                                                                <Error Type="118" Code="118">Opps! unable to update
                                                                </Error>
                                                            </Errors>
                                                        </OTA_HotelRestrictionInfoRS>';
                                                        return response($return_array)->header('Content-Type',
                                                        ...
                                                        <=',$start_date)->where('date_to','>=',$end_date)->first();
                                                            ...
                                                            $get_number_of_room =
                                                            Inventory::select('no_of_rooms')->where($condition)->where('date_from','>=',$start_date)->where('date_to','
                                                            <=',$end_date)->first();
                                                                if(sizeof($get_number_of_room)>0){
                                                                $data=['date_from'=>$start_date,'date_to'=>$end_date,'room_type_id'=>$room_type_id,'user_id'=>0,'ota_id'=>0,'hotel_id'=>$hotel_id,'no_of_rooms'=>$get_number_of_room->no_of_rooms,'pms_id'=>$pms_id,'pms_name'=>$pms_name,'client_ip'=>$ip,'los'=>$los,'ota_details'=>'be'];
                                                                $user_id=$data['user_id'];
                                                                ...
                                                                <OTA_HotelRestrictionInfoRS
                                                                    ...
                                                                    Version="'.$version.'">
                                                                    <Errors xmlns="">
                                                                        <Error Type="118" Code="118">Opps! unable to
                                                                            update</Error>
                                                                    </Errors>
                                                                </OTA_HotelRestrictionInfoRS>';
                                                                return response($return_array)->header('Content-Type',
                                                                ...
                                                                <OTA_HotelRestrictionInfoRS
                                                                    ...
                                                                    Version="'.$version.'">
                                                                    <Errors xmlns="">
                                                                        <Error Type="118" Code="118">InvalidRatePlan
                                                                        </Error>
                                                                    </Errors>
                                                                </OTA_HotelRestrictionInfoRS>';
                                                                return response($return_array)->header('Content-Type',
                                                                ...
                                                                <OTA_HotelRestrictionInfoRS
                                                                    ...
                                                                    Version="'.$version.'">
                                                                    <Errors xmlns="">
                                                                        <Error Type="118" Code="118">Please set key in
                                                                            header</Error>
                                                                    </Errors>
                                                                </OTA_HotelRestrictionInfoRS>';
                                                                return response($return_array)->header('Content-Type',
                                                                ...
                                                                <OTA_HotelRestrictionInfoRS
                                                                    ...
                                                                    Version="'.$version.'">
                                                                    <Errors xmlns="">
                                                                        <Error Type="118" Code="118">Please set
                                                                            Hotelcode<< /Error>
                                                                    </Errors>
                                                                </OTA_HotelRestrictionInfoRS>';
                                                                return response($return_array)->header('Content-Type',
                                                                ...
                                                                $this->PmsComponents->serviceRequest($api_key,
                                                                $hotel_id, $ip, 'Fetch Hotels');

                                                                $HotelDetails =
                                                                $this->PmsComponents->searchHotelIdwise($hotel_id);
                                                                $Room_Types =
                                                                $this->PmsComponents->searchRoomtypeHotelwise($hotel_id);
                                                                $RoomTypes = array();
                                                                $count = 0;
                                                                foreach ($Room_Types as $rt) {
                                                                $room_type=$rt->room_type_id;
                                                                $ratePlans =
                                                                $this->PmsComponents->SearchRoomratePlanHotelWise($room_type);
                                                                $RoomTypes[] = array(
                                                                ...
                                                                if(!empty($result)){
                                                                $return_array = array('data'=>$result, 'status'=>'yes');
                                                                //if yes then only dashboard or no only service
                                                                }
                                                                else{
                                                                $return_array =
                                                                array("status"=>"Error","code"=>"401","message"=>"Hotel
                                                                not found");
                                                                }
                                                                }
                                                                return $return_array;
                                                                ...
                                                                $return_array =
                                                                array("status"=>"Error","code"=>"401","message"=>"Fields
                                                                are blank");
                                                                return $return_array;
                                                                }
                                                                }
                                                                ...
                                                                $hotel_id = $data['hotel_id'];
                                                                $last_booking_id =
                                                                isset($data['last_id'])?$data['last_id']:'';
                                                                $booking_date_i =
                                                                isset($data['booking_date'])?$data['booking_date']:'';
                                                                $ip = $this->ipAddress->getIPAddress();
                                                                $checkApi = $this->PmsComponents->checkApi($api_key);
                                                                $hotels = explode(',',$checkApi->hotels);
                                                                ...
                                                                $this->PmsComponents->serviceRequest($api_key,
                                                                $hotel_id, $ip, 'Fetch Bookings');

                                                                //Fetch Booking Details
                                                                $BookingDetails =
                                                                $this->PmsComponents->searchAllBookings($hotel_id,
                                                                $last_booking_id, $booking_date_i);
                                                                $all_bookings = array();
                                                                foreach ($BookingDetails as $bd){
                                                                $rooms =array();
                                                                $user_id
                                                                =isset($bd->user_id)?$bd->user_id:$bd['user_id'];
                                                                $invoice_id
                                                                =isset($bd->invoice_id)?$bd->invoice_id:$bd['invoice_id'];
                                                                $ref_no =isset($bd->ref_no)?$bd->ref_no:$bd['ref_no'];

                                                                $User_Details =
                                                                $this->PmsComponents->UserInfo($user_id);
                                                                $Booked_Rooms =
                                                                $this->PmsComponents->NoOfBookings($invoice_id);
                                                                $check_out =
                                                                isset($Booked_Rooms[0]->check_out)?$Booked_Rooms[0]->check_out:$Booked_Rooms[0]['check_out'];
                                                                $check_in =
                                                                isset($Booked_Rooms[0]->check_in)?$Booked_Rooms[0]->check_in:$Booked_Rooms[0]['check_in'];
                                                                ...
                                                                $booking_date
                                                                =isset($bd->booking_date)?$bd->booking_date:$bd['booking_date'];
                                                                $booking_id =date("dmy",
                                                                strtotime($booking_date)).str_pad($invoice_id, 4, '0',
                                                                STR_PAD_LEFT);
                                                                ...
                                                                $room_details =
                                                                isset($bd->room_type)?$bd->room_type:$bd['room_type'];
                                                                $room_type_plan=explode(",", $room_details);
                                                                $plan= array();
                                                                for($i=0; $i<sizeof($room_type_plan); $i++){
                                                                    $plan[]=substr($room_type_plan[$i], -5, -2); }
                                                                    $extra_details=isset($bd->
                                                                    extra_details)?$bd->extra_details:$bd['extra_details'];
                                                                    ...
                                                                    $room_type_id =
                                                                    isset($br->room_type_id)?$br->room_type_id:$br['room_type_id'];
                                                                    $rooms_no =
                                                                    isset($br->rooms)?$br->rooms:$br['rooms'];
                                                                    if(trim($room_type_id)==trim($key)){
                                                                    for($j=0;$j<$rooms;$j++){
                                                                        $adult=$adult+$value[$j][0];
                                                                        $child=$child+$value[$j][1]; } } }
                                                                        if($child=='NA' ){ $child=0; }
                                                                        $total_amount=isset($bd->
                                                                        total_amount)?$bd->total_amount:$bd['total_amount'];
                                                                        ...
                                                                        "room_rate" =>
                                                                        ($total_amount/$rooms_no)/$no_of_nights,
                                                                        "plan" => trim($plan[$k]),
                                                                        "adult" => $adult,
                                                                        "child" => $child
                                                                        );
                                                                        ...
                                                                        $user_fname =
                                                                        isset($User_Details->first_name)?$User_Details->first_name:$User_Details['first_name'];
                                                                        $user_lname =
                                                                        isset($User_Details->last_name)?$User_Details->last_name:$User_Details['last_name'];
                                                                        $user_info = array(
                                                                        "user_name" => $user_fname.' '.$user_lname,
                                                                        "mobile" =>
                                                                        isset($User_Details->mobile)?$User_Details->mobile:$User_Details['mobile'],
                                                                        "email" =>
                                                                        isset($User_Details->email_id)?$User_Details->email_id:$User_Details['email_id'],
                                                                        );

                                                                        $Bookings = array(
                                                                        "date_of_booking" => $booking_date,
                                                                        "hotel_id" =>
                                                                        isset($bd->hotel_id)?$bd->hotel_id:$bd['hotel_id'],
                                                                        "hotel_name" =>
                                                                        isset($bd->hotel_name)?$bd->hotel_name:$bd['hotel_name'],
                                                                        "check_in" =>
                                                                        isset($Booked_Rooms[0]->check_in)?$Booked_Rooms[0]->check_in:$Booked_Rooms[0]['check_in'],
                                                                        "check_out" =>
                                                                        isset($Booked_Rooms[0]->check_out)?$Booked_Rooms[0]->check_out:$Booked_Rooms[0]['check_out'],
                                                                        ...
                                                                        "grand_total" =>
                                                                        isset($bd->total_amount)?$bd->total_amount:$bd['total_amount'],
                                                                        "paid_amount" =>
                                                                        isset($bd->paid_amount)?$bd->paid_amount:$bd['paid_amount'],
                                                                        "channel" => "Bookingjini",
                                                                        "status" => "confirmed"
                                                                        );
                                                                        ...
                                                                        $OTABookingDetails =
                                                                        $this->PmsComponents->searchAllOTABookings($hotel_id,
                                                                        $booking_date_i);
                                                                        ...
                                                                        $ota_booking_id =date("dmy",
                                                                        strtotime($ota_booking_date)).str_pad($ota_invoice_id,
                                                                        4, '0', STR_PAD_LEFT);


                                                                        $plans=$this->PmsComponents->getHotelRatePlanIdFromRatePlanSynch($otabd->room_type,
                                                                        $otabd->rate_code, $hotel_id);
                                                                        $rooms1=$this->PmsComponents->getHotelRoomIdFromRoomSynch($otabd->room_type,
                                                                        $hotel_id);
                                                                        // if(sizeof($plans)<=0) // { // $plans='NA' ;
                                                                            // } $adults=explode(",",$otabd->
                                                                            no_of_adult);
                                                                            ...
                                                                            $plans_name =
                                                                            isset($plans[0][$k])?$plans[0][$k]:'NA';
                                                                            $plan_id =
                                                                            isset($plans[0][$k+1])?$plans[0][$k+1]:'NA';
                                                                            $ota_rooms[] = array(
                                                                            "room_type_id" => $key,
                                                                            "room_type_name" => $value,
                                                                            "no_of_rooms" => $room_qty[$k],
                                                                            "room_rate" =>
                                                                            ($amount/$room_qty[$k])/$no_of_nights,
                                                                            ...
                                                                            if($bookings){
                                                                            $return_array =array('data'=>$bookings,
                                                                            'b_status'=>'yes'); //$all_bookings; //if
                                                                            yes then only dashboard or no only service
                                                                            }
                                                                            else{
                                                                            if($last_booking_id==''){
                                                                            $return_array =
                                                                            array("status"=>"Empty","code"=>"200","message"=>"Last
                                                                            booking id is not valid or no more booking
                                                                            is there.");
                                                                            }
                                                                            else{
                                                                            $return_array =
                                                                            array("status"=>"Empty","code"=>"200","message"=>"There
                                                                            is no new bookings");
                                                                            }
                                                                            }
                                                                            return $return_array;
