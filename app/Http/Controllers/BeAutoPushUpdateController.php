...
class BeAutoPushUpdateController extends Controller
{
    protected $inventoryService;
    private $updateInvService;
    protected $idsService, $ipService, $cmBookingDataInsertion,$WinhmsController;
    public function __construct(CmOtaBookingInvStatusService $cmOtaBookingInvStatusService, IdsController $idsService, IpAddressService $ipService, CmBookingDataInsertionController $cmBookingDataInsertion,WinhmsController $WinhmsController)
    {
    ...
    public function bookingjiniUpdate($bucket_data, $booking_data)
    {
        ...
        foreach ($room_types as $key => $room_type)
        {
            ...
            if ($booking_data['booking_source'] != 'crs')
            {
            ...
            if (!empty($room_type))
            {
                $log_data = ["action_id" => 4, "hotel_id" => $bucket_hotel_id, "ota_id" => $bucket_ota_id, "booking_ref_id" => $bucket_ota_booking_tabel_id, "user_id" => 0, "request_msg" => '', "response_msg" => '', "request_url" => '', "status" => 2, "ip" => $booking_ip, "comment" => "Processing for update "];
                $inventoryData = array();
                $inventoryData = $this
                    ->cmOtaBookingInvStatusService
                    ->getInventoryDetailsByRoomType($bucket_ota_booking_tabel_id, $room_type, $bucket_hotel_id, $bucket_ota_id); // 0 => No of days prior to the booking
                $serach_be_flag = 1;
            ...
            else
            {
                ...
                $log_data = ["action_id" => 4, "hotel_id" => $bucket_hotel_id, "ota_id" => $bucket_ota_id, "booking_ref_id" => $bucket_ota_booking_tabel_id, "user_id" => 0, "request_msg" => '', "response_msg" => '', "request_url" => '', "status" => 0, "ip" => $booking_ip, "comment" => "Booking room type is not mapped."];
    ...
    public function agodaUpdate($bucket_data, $booking_data)
    {
        ...
        $ota_details_data = $cmOtaDetailsModel->where('hotel_id', '=', $bucket_hotel_id)->where('ota_id', '=', $bucket_ota_id)->first();
    ...
    public function goibiboUpdate($bucket_data, $booking_data)
    {
        ...
        foreach ($room_types as $key => $room_type)
        {
            ...
            {
                // set log for Booking Room Type is not synch with hotel Room Type.
                $log_data = ["action_id" => 4, "hotel_id" => $bucket_hotel_id, "ota_id" => $bucket_ota_id, "booking_ref_id" => $bucket_ota_booking_tabel_id, "inventory_ref_id" => '', "rate_ref_id" => '', "user_id" => 0, "request_msg" => '', "response_msg" => '', "request_url" => '', "status" => 0, "ip" => $booking_ip, "comment" => "Booking room type is not mapped."];
                $logModel->fill($log_data)->save();
            } // If else !empty($result) closed here.
        ...
        foreach ($room_types as $key => $room_type)
        {
            ...
            if (!empty($room_type))
            {
                ...
                $inventoryData = $this
                    ->cmOtaBookingInvStatusService
                    ->getInventoryDetailsByRoomType($bucket_ota_booking_tabel_id, $room_type, $bucket_hotel_id, $bucket_ota_id); // 0 => No of days prior to the booking
                $ratePlanTypeSynchronizeData = CmOtaRatePlanSynchronize::select('*')->where('ota_room_type_id', '=', $room_code)->first();
                $url = '';
                if (!isset($ratePlanTypeSynchronizeData->ota_rate_plan_id))
                {
                    $log_data = ["action_id" => 4, "hotel_id" => $bucket_hotel_id, "ota_id" => $bucket_ota_id, "booking_ref_id" => $bucket_ota_booking_tabel_id, "user_id" => 0, "request_msg" => '', "response_msg" => '', "request_url" => '', "status" => 0, "ip" => $booking_ip, "comment" => "Booking rate plan is not mapped."];
                    $logModel->fill($log_data)->save();
                    $rateplan_code = 0;
                }
                else
    ...
    public function testhandleIds(Request $request)
    {
        ...
        $this->handleIds(array() , array(
            'booking_hotel_id' => 1151
        ));
    }
    public function handleIds($bucket_data, $booking_data)
    {
        ...
        if ($ids_status)
        {
            ...
            if ($booking_data['booking_source'] == 'crs')
            {
                ...
                $booking_id = date("dmy", strtotime($booking_data['booking_date'])) . str_pad($ids_data['booking_id'], 4, '0', STR_PAD_LEFT);
                $ids_data['booking_id'] = $booking_id;
            }
            else
            {
            ...
            $last_ids_id = $this
                ->idsService
                ->idsBookings($hotel_id, $type, $ids_data, $customer_data, $bucket_booking_status);
            if ($last_ids_id)
            {
                DB::table('cm_ota_booking')->where('id', $bucket_data['bucket_ota_booking_tabel_id'])->update(['ids_re_id' => $last_ids_id]);
                $ids_string = $this
                    ->idsService
                    ->getIdsString($last_ids_id);
                if ($this
                    ->idsService
                    ->pushReservations($ids_string, $last_ids_id))
                {
                    DB::table('ids_reservation')->where('id', $last_ids_id)->update(['ids_confirm' => 1]);
                }
                else
                {
                    DB::table('ids_reservation')
                        ->where('id', $last_ids_id)->update(['ids_confirm' => 2]);
                }
            }
    ...
    public function handleWinhms($bucket_data, $booking_data)
    {
        ...
        if ($winhms_status)
        {
            ...
            {
                $name = $customer_details;
                $booking_id = date("dmy", strtotime($booking_data['booking_date'])) . str_pad($winhms_data['booking_id'], 4, '0', STR_PAD_LEFT);
                $winhms_data['booking_id'] = $booking_id;
            }
            else
            {
                $name = explode(' ', $customer_details[0]);
            ...
            if ($last_winhms_id)
            {
                DB::table('cm_ota_booking')->where('id', $bucket_data['bucket_ota_booking_tabel_id'])->update(['winhms_re_id' => $last_winhms_id]);
    ...
    public function prepare_winhms_data($hotel_id, $ota_booking_data)
    {
        ...
        foreach ($room_types as $key => $room_type)
        {
            if ($ota_booking_data['booking_source'] != 'crs')
            {
            ...
            for ($i = 0;$i < $rooms_qty[$key];$i++)
            {
                ...
                $arr = array(
                    ...
                    'rate_plan_id' => $hotel_rate_plan_id,
                    'adults' => $total_adult,
                    'from_date' => $from_date,
                    'to_date' => $to_date,
                    'rates' => $rates_arr
    ...
    public function handleBookOne($ota_booking_tabel_id, $hotel_id)
    {
        if ($hotel_id == 1491)
        {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                'key: b5a45e2b-e1a5-4707-93d0-cbd5cfe902f0'
            ));
            ...
            if (isset($all_bookings->data))
            {
                ...
                {
                    $id = substr($booking
                        ->BookingsDetails->booking_id, 6);
                    if ($ota_booking_tabel_id == $id)
                    {
                        $tr_id = $booking
                            ->BookingsDetails->booking_id;
                        $return_array = array(
                            'data' => array(
                                $booking
                            ) ,
    ...
    public function mailHandler($ota_booking_id, $bucket_booking_status)
    {
        ...
        $hotel = HotelInformation::where('hotel_id', $hotel_id)->select('hotel_name', 'email_id', 'hotel_address', 'city_id', 'state_id')
            ->first();
        $city_name = DB::table('city_table')->where('city_id', $hotel->city_id)
            ->first();
        $state_name = DB::table('state_table')->where('state_id', $hotel->state_id)
            ->first();
        ...
        $ota_details = CmOtaDetails::where('ota_id', $ota_id)->select('ota_name', 'commision')
            ->first();
        ...
        $template = array(
            ...
            "rate_plan" => $hotel_rate_plan,
            "rooms" => $rooms,
            "total_amount" => $ota_booking_data->amount,
            "customer_details" => $ota_booking_data->customer_details,
            "booking_status" => $bucket_booking_status,
    ...
    */
    public function sendMail($hotel_email, $template, $subject, $hotel_name, $email_id1)
    {
        ...
        {
            $message->to($data['hotel_email'])->bcc($data['mail_array'])->from(env("MAIL_FROM") , $data['hotel_name'])->subject($data['subject']);
        });
        if (Mail::failures())
        {
            return false;
    ...
    public function mailHandler1($ota_booking_id, $bucket_booking_status)
    {
        ...
        $hotel = HotelInformation::where('hotel_id', $hotel_id)->select('hotel_name', 'email_id', 'hotel_address', 'city_id', 'state_id')
            ->first();
        $city_name = City::where('city_id', $hotel->city_id)
            ->first();
        $state_name = State::where('state_id', $hotel->state_id)
            ->first();
        $hotel_name = $hotel->hotel_name;
        $email_id = explode(',', $hotel->email_id);
        if ($email_id)
    ...
    public function sendMail1($hotel_email, $template, $subject, $hotel_name, $email_id1)
    {
        ...
        {
            $message->to($data['hotel_email'])->bcc($data['mail_array'])->from(env("MAIL_FROM"))
