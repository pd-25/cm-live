...
class OtaAutoPushUpdateTestControllers2 extends Controller{
		protected $inventoryService;
		private $updateInvService;
		protected $idsService,$ipService,$cmBookingDataInsertion,$ktdcService;
    public function __construct(CmOtaBookingInvStatusService $cmOtaBookingInvStatusService,IdsController $idsService,IpAddressService $ipService,CmBookingDataInsertionController $cmBookingDataInsertion,KtdcController $ktdcService)
    {
...
public function handleKtdc($bucket_data,$booking_data,$booking_channel)
{
    ...
    if($ktdc_status)
    {
		...
		$customer_data['mobile']=$customer_details[2];
		// $name=explode(' ',$customer_details[0]);
		// $customer_data['first_name']=$customer_details[0];
		// $customer_data['last_name']=$customer_details[1];
		// $customer_data['email_id']=$customer_details[2];
		// $customer_data['mobile']=$customer_details[3];
		$type=$booking_data['booking_channel'];
		$ktdc_string="";
...
public function sendMail($hotel_email,$template, $subject,$hotel_name, $email_id1,$hotel_id)
{
	...
	{
		if($hotel_id == 2280){
			$mail_array=['channelmanager@5elements.co.in', $email_id1,'centralreservations@ktdcbooking.com','interface@datadevices.com'];
		}
		else{
	...
	{
		if($hotel_id == 2280){
			$mail_array=['channelmanager@5elements.co.in','centralreservations@ktdcbooking.com','interface@datadevices.com'];
		}
		else{
