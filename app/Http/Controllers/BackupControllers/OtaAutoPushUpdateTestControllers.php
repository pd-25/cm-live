				...
				if($inventoryData[$i]->block_status==0)
				{
					...
					$status =$ary_data['status']['code'];
						if (substr($status, 0, 1) === 'S') {
							...
							$inv_data=array('hotel_id'=>$bucket_hotel_id,'room_type_id'=>$inventoryData[$i]->room_type_id,'no_of_rooms'=>$room_quanty,'date_from'=>$stDate1,'date_to'=>$edDate1,'channel'=>$bucket_data['bucket_ota_name'],'client_ip'=>$booking_ip,'user_id'=>0,'multiple_days'=>$multiple_days,'block_status'=>$inventoryData[$i]->block_status,'los'=>$inventoryData[$i]->los,'ota_booking_id'=>$bucket_ota_booking_tabel_id,'booking_status'=>$bucket_booking_status);
