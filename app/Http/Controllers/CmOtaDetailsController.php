    ...
    **/
    public function addNewCmHotel(Request $request)
    {
        ...
        }elseif($data['ota_name']=='ClearTripHyperGuest')
        {
         ...
         //p_d   13april
         elseif($data['ota_name']=='Simplotel')
         {
             $data['auth_parameter']='{"ID":"C92AC80NKX65C0752A8703V421K8H2E","ID_Context":"AL97G8JJ8592921F014Y5I2B8H2641A","MessagePassword":"P90KM86NJU74M0861G5613S8812BJ72"}';
         }
...
/**
    ...
    **/
    public function multipleFunction(int $ota_id, Request $request)
    {
        ...
        elseif($ota_data->ota_name == "Simplotel")
        {
            $deleteRoomType    = CmOtaRoomTypeFetch::where('hotel_id',$ota_data->hotel_id)->where('ota_id',$ota_id)->delete();
            $deleteRatePlan    = CmOtaRateTypeFetch::where('hotel_id',$ota_data->hotel_id)->where('ota_id',$ota_id)->delete();
           return $this->simplotelCmHotel($ota_data->ota_hotel_code,$ota_data->url,$auth_parameter,$ota_data->hotel_id,$ota_id,$ota_data->ota_name);
        }
    ...
    public function clearTripHyperGuestCmHotel($ota_hotel_code,$commonurl,$auth_parameter,$hotel_id,$ota_id,$ota_name)
    {
        ...
        $validate_to   = "";
        $rate_active   = 1;
        if(isset($room_type_deatils_array['RoomTypes']['RoomType']['@attributes'])){    //single room type
            ...
            if(isset($room_type_deatils_array['RatePlans']['RatePlan']['@attributes'])){
                ...
                if(isset($room_type_deatils_array['RoomTypes']['RoomType'])){   //single room types
                    ...
                    //echo "<pre>"."demo";print_r($room_type_deatils_array);exit;
                    $j = 0;
    ...
    //p_d  13april simplotel
    public function simplotelCmHotel($ota_hotel_code,$commonurl,$auth_parameter,$hotel_id,$ota_id,$ota_name){
        $id =              trim($auth_parameter->ID);
        ...
        $headers           = array (
            ...
            EchoToken="sdf54-sdf65-sdf98652" SummaryOnly="true"
            TimeStamp="2017-09-26T09:23:47Z">
            <POS>
            <Source>
            ...
            $url = $commonurl.'fetch_roomtypes_info_or_rates';
            $response = $curlService->to($url)->withData($xml)->post();
    
            $array_data = json_decode(json_encode(simplexml_load_string($xml)),true);
            //dd($array_data);
             if(empty($array_data)){
            $res=array('status'=>0,"message"=>"simplotel Room type and Room Rate fetch failed","err"=>"No Data Found");
            return response()->json($res);
            }
            $hotelCode = $array_data['RatePlans']['@attributes']['HotelCode'];
            $roomTypes2 = $array_data['RatePlans']['RatePlan'];

            foreach($roomTypes2 as $ota_room_type){
             $r = $ota_room_type['Rates']['Rate'];
                foreach($r as $k=>$v){
                   // dd($v);
                  
                $ota_room_type_id=$v['@attributes']['InvTypeCode'];
                $ota_room_type_name=$v['@attributes']['InvCode'];
                $data['hotel_id'] = $hotelCode;
                $data['ota_id']   = $ota_id;
                $data['ota_name']         = $ota_name;          
                $data['ota_room_type_id']         = $ota_room_type_id;          
                $data['ota_room_type_name']         = $ota_room_type_name;          
                $data['active']         = 1;  
                 //$datas[]= $data;     
                /*----------------cmOtaRoomTypeFetchModel Save -------------------*/
                $insert_roomType = CmOtaRoomTypeFetch::create($data);
                }
                //dd($datas);   
            }

                $rateplan = $array_data['RatePlans']['RatePlan'];
                //  dd($roomTypes);
                        $rate_plans=[];
                        $room_type=[];
                foreach($rateplan as $k=>$value1){
                       $roomTypes2 = $value1['Rates']['Rate'];
                       foreach($roomTypes2 as $key => $value){
                        $ota_room_type_id=$value['@attributes']['InvTypeCode'];
                        $ota_room_type_name=$value['@attributes']['InvCode']; 
                        //rate type name is not define
                       $rate_plan['ota_rate_type_id']=$value1['@attributes']['RatePlanCode'];
                       $rate_plan['ota_rate_type_name']=$value1['Description']['@attributes']['Name'];
                       $rate_plan['hotel_id'] = $hotelCode;
                       $rate_plan['ota_id'] = 12;
                       $rate_plan['ota_name'] = 'Simplotel';
                       $rate_plan['ota_room_type_id'] = $ota_room_type_id;
                       $rate_plan['ota_room_type_name'] = $ota_room_type_name;
                       $rate_plan['ota_rate_type'] = $value1['Description']['Text'];
                       $rate_plan['validate_from'] = '12.04.2022';
                       $rate_plan['validate_to'] = '12.04.2022';
                       $rate_plan['active'] = 1;
                       $rate_plans[] =$rate_plan; 
                       $insert_rateType = CmOtaRateTypeFetch::create($rate_plans); 
                       }
                }
                 //dd($rate_plans);
                //  dd($rate_plan);
                        ...
                        $res=array('status'=>1,"message"=>"simplotel Room type and Room Rate fetch successfull");
                        return response()->json($res);
    }
