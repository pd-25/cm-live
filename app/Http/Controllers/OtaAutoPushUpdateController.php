...
class OtaAutoPushUpdateController extends Controller{
		protected $inventoryService;
		private $updateInvService;
		protected $idsService,$ipService,$cmBookingDataInsertion;
		// protected $ktdcService;
    public function __construct(CmOtaBookingInvStatusService $cmOtaBookingInvStatusService,IdsController $idsService,IpAddressService $ipService,CmBookingDataInsertionController $cmBookingDataInsertion,KtdcController $ktdcService,WinhmsController $winhmscontroller)
    {
	...
	public function bookingjiniUpdate($bucket_data,$booking_data){
		...
		echo "<pre>";print_r($this->handleWinhms($bucket_data,$booking_data));exit;
		$booking_ip='1.1.1.1';
		...
		// }
		DB::table('cmlive.booking_logs')->where('id', $logModel->id)
...
public function prepare_winhms_data($hotel_id,$ota_booking_data)// change according to live code.
{
	$cmOtaRoomTypeSynchronizeModel  = new CmOtaRoomTypeSynchronize();
	$cmOtaRatePlanSynchronizeModel 	= new CmOtaRatePlanSynchronize();
    $booking_data=array();
	...
	$no_of_room_types=sizeof($room_types);
    	foreach($room_types as $key=>$room_type)
        {
            ...
            $arr=array('room_type_id'=>$room_type,'rate_plan_id'=>$hotel_rate_plan_id,'adults'=>$total_adult,'from_date'=>$from_date,'to_date'=>$to_date,'rates'=>$rates_arr,'no_of_rooms'=>$rooms_qty[$key]);
			array_push($booking_data['room_stay'],$arr);
			}
		}
    return $booking_data;
...
public function mailHandler($ota_booking_id,$bucket_booking_status)
{
	...
	"city_name"=>$city_name->city_name,"state_name"=>$state_name->state_name,"booking_id"=>$ota_booking_data->unique_id,"check_in"=>$ota_booking_data->checkin_at,"check_out"=>$ota_booking_data->checkout_at,"room_type"=>$hotel_room_type,"rate_plan"=>$hotel_rate_plan,"rooms"=>$rooms,"total_amount"=>$ota_booking_data->amount,"customer_details"=>$ota_booking_data->customer_details,"booking_status"=>$bucket_booking_status,"payment_status"=>$ota_booking_data->payment_status,"commision_amount"=>$commision_amount,"supplier_amount"=>$supplier_amount,"number_of_nights"=>$number_of_nights);
	if($email_id)
	{
...
public function sendMail($hotel_email,$template, $subject,$hotel_name, $email_id1,$hotel_id)
{
	...
	{
		if($hotel_id == 2280){
			$mail_array=['channelmanager@5elements.co.in', $email_id1,'centralreservations@ktdcbooking.com'];
		}
		else{
	...
	{
		if($hotel_id == 2280){
			$mail_array=['channelmanager@5elements.co.in','centralreservations@ktdcbooking.com'];
		}
		else{
...
public function mailHandler1($ota_booking_id,$bucket_booking_status)
{
	...
	$template=array("ota_name"=>$channel_name,"hotel_name"=>$hotel->hotel_name,"hotel_address"=>$hotel->hotel_address,"city_name"=>$city_name->city_name,"booking_id"=>$ota_booking_data->unique_id,"check_in"=>$ota_booking_data->checkin_at,"check_out"=>$ota_booking_data->checkout_at,'booking_date'=>$ota_booking_data->booking_date,"room_type"=>$hotel_room_type,"rate_plan"=>$hotel_rate_plan,"rooms"=>$rooms,"total_amount"=>$ota_booking_data->amount,"customer_details"=>$customer_name,"booking_status"=>$bucket_booking_status,"payment_status"=>$ota_booking_data->payment_status,"number_of_nights"=>$number_of_nights,"state_name"=>$state_name->state_name,"customer_number"=>$customer_number,"tax_amount"=>$tax_amount,"inclusion"=>$inclusion,"special_info"=>$special_info,"cancel_policy"=>$cancel_policy,"no_of_adult"=>$guest,"no_of_child"=>$guest1,"customer_email"=>$customer_email,"currency"=>$currency);
	if($email_id)
	{
...
public function handleKtdc($bucket_data,$booking_data,$booking_channel)
{
    ...
    if($ktdc_status)
    {
		...
		if($last_ktdc_id)
        {
			DB::table('cm_ota_booking')->where('id',$bucket_data['bucket_ota_booking_tabel_id'])->update(['ktdc_re_id' => $last_ktdc_id]);
			$ktdc_string=$this->ktdcService->getKtdcString($last_ktdc_id);
			if($this->ktdcService->pushReservations($ktdc_string,$last_ktdc_id)){
				DB::table('ktdc_reservation')->where('id', $last_ktdc_id)->update(['ktdc_confirm' => 1]);
			}else{
				DB::table('ktdc_reservation')->where('id', $last_ktdc_id)->update(['ktdc_confirm' => 2]);
			}
        }
...
public function prepare_ktdc_data($hotel_id,$ota_booking_data)
{
    ...
    foreach($room_types as $key=>$room_type)
    {
        ...
        $rates_arr=array();
		$total_adult=2;//By default adults 2
		$display_total_adult+= $total_adult;
        for($i=0;$i<$rooms_qty[$key];$i++)
        {
            $frm_date=$from_date;
            $rates_arr=array();
        ...
        $arr=array('room_type_id'=>$room_type,'rate_plan_id'=>$hotel_rate_plan_id,'adults'=>$total_adult,'from_date'=>$from_date,'to_date'=>$to_date,'rates'=>$rates_arr,'no_of_nights'=>$rooms_qty[$key]);
		array_push($booking_data['room_stay'],$arr);
		}
	}
