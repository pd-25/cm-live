...
class CRMBookingControllers extends Controller
{
    public function getBooking(Request $request){
        ...
        if(strlen($email_id) > 0){
            ...
            $getBeBooking =  DB::table('user_table')
                            ->join('invoice_table','user_table.user_id','=','invoice_table.user_id')
            ...
            foreach($getBeBooking as $be_booking){
                ...
                $get_room_type = DB::table("room_type_table")
                                ->select('room_type')
            ...
            foreach($geOTABooking as $ota_data){
                ...
                foreach($room_type_id as $room){
                    $getRoom_name  = DB::table('cm_ota_room_type_synchronize')
                                    ->join('room_type_table','cm_ota_room_type_synchronize.room_type_id','=','room_type_table.room_type_id')
                ...
                foreach($rate_plan_id as $rate){
                    $get_rate_plan_name  = DB::table('cm_ota_rate_plan_synchronize')
                                    ->join('rate_plan_table','cm_ota_rate_plan_synchronize.hotel_rate_plan_id','=','rate_plan_table.rate_plan_id')
        ...
        else{
            {
                ...
                $getBeBooking =  DB::table('user_table')
                                ->join('invoice_table','user_table.user_id','=','invoice_table.user_id')
                                ->join('hotel_booking','invoice_table.invoice_id','=','hotel_booking.invoice_id')
                                ->select('hotel_booking.check_in','hotel_booking.check_out','hotel_booking.room_type_id','hotel_booking.rate_plan_id','hotel_booking.hotel_booking_id','hotel_booking.rooms','invoice_table.hotel_id','invoice_table.invoice_id','invoice_table.hotel_name','invoice_table.total_amount','invoice_table.extra_details','invoice_table.paid_amount','invoice_table.booking_date','invoice_table.booking_status','user_table.first_name','user_table.last_name','user_table.user_id','user_table.email_id','user_table.address','user_table.mobile')
                                ->where('invoice_table.hotel_id',$hotel_id)
                                ->where('invoice_table.booking_status',1)
                                ->where('user_table.email_id',$contact)
                                ->get();
                foreach($getBeBooking as $be_booking){
                    $get_room_type = DB::table("room_type_table")
                                    ->select('room_type')
                                    ->where('room_type_id',$be_booking->room_type_id)
                                    ->first();
                    $get_rate_plan = DB::table('rate_plan_table')
                                    ->select('plan_type','plan_name')
                                    ->where('rate_plan_id',$be_booking->rate_plan_id)
                                    ->first();
                    $booking_id     =date("dmy", strtotime($be_booking->booking_date)).str_pad($be_booking->invoice_id, 4, '0', STR_PAD_LEFT);
                    $get_extra_details = json_decode($be_booking->extra_details);
                    $adult=0;
                    ...
                    $room_info = array(
                        ...
                        "room_type"         => $get_room_type->room_type,
                        "no_of_rooms"       => $be_booking->rooms,
                        "rate_plan_type"    => $get_rate_plan->plan_type,
                        "plan_name"         => $get_rate_plan->plan_name,
                        "adult"             => $adult,
                        "child"             => $child
                    );
                    $user_info = array(
                ...
                $geOTABooking = DB::select(DB::raw("select hotel_id,customer_details,booking_status,rooms_qty,room_type,checkin_at,checkout_at,booking_date,rate_code,amount,channel_name,no_of_adult,no_of_child,payment_status,unique_id from cm_ota_booking where hotel_id = $hotel_id and confirm_status = 1 and cancel_status = 0 and customer_details LIKE '%$contact%'"));
