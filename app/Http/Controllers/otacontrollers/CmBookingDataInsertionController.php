...
class CmBookingDataInsertionController extends Controller
{
    public function cmBookingDataInsertion($booking_hotel_id,$bucket_ota_booking_tabel_id){
        ...
        if(!in_array($booking_hotel_id,$hotel_ids)){
            ...
            if($getCmBookingData){
                ...
                {
                    //get room_type_id //
                    $get_room_type = CmOtaRoomTypeSynchronize::join('cm_ota_room_type_fetch','cm_ota_room_type_synchronize.ota_room_type','=','cm_ota_room_type_fetch.ota_room_type_id')->join('kernel.room_type_table','room_type_table.room_type_id','=','cm_ota_room_type_synchronize.room_type_id')->select('room_type_table.room_type_id','ota_type_id','ota_room_type',
                    ...
                    for($j=0;$j<$rooms[$i];$j++)
                    {
                        ...
                        $CmBookingDetails->room_type             = trim($get_room_type->room_type);
                        ...
                        $CmBookingDetails->room_type_id          = trim($get_room_type->room_type_id);
                        ...
                        if($no_of_child == '0'){
                            ...
                            $CmBookingDetails->child             = $no_of_child_arr[$i];
                        }
                ...
                if($db_status)
                {
                    $this->pushBookingToGems($getCmBookingData->unique_id,true);
    ...
    public function pushBookingToGems($unique_id,$gems){
        ...
        'cm_ota_booking.payment_status','cm_ota_booking.booking_status','cm_ota_details.ota_name','cm_ota_booking.tax_amount','cm_ota_booking.channel_name')
        ...
        foreach($getBookingDetails as $key => $bk_details){
            ...
            $booking_id     =date("dmy", strtotime($booking_date)).str_pad($unique_id, 4, '0', STR_PAD_LEFT);
            ...
            $user_info = array(
                ...
                if($gems == 'true'){
                  if($bk_details->channel_name == "Booking.com"){
