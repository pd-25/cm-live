...
*   @29/01/19
**/
class TravelguruController extends Controller
{
    protected $cmOtaBookingInvStatusService;
    protected $bookingData;
    protected $curlcall;
    protected $instantbucket;
    ...
    {
        $logpath = storage_path("logs/travelguru.log".date("Y-m-d"));
            ...
            <OTA_ReadRQ Version="5.001" xmlns="http://www.opentravel.org/OTA/2003/05">
                ...
                <ReadRequests>
                    <HotelReadRequest HotelCode="'.$ota_hotel_code.'">
                        <SelectionCriteria Start="'.$fromDate.'T00:00:00" End="'.$toDate.'" />
                    </HotelReadRequest>
                </ReadRequests>
            </OTA_ReadRQ>';
        ...
        if (strtoupper($booking_status)=='CANCELLED') {
            ...
            $otaBookingData=CmOtaBooking::where('unique_id', $uniqueID)->where('confirm_status', 1)->where('cancel_status',
            1)->first();
                if (!$otaBookingData) {
        ...
        if (isset($customer_info_array['ContactPerson'][7]['@attributes']['ContactType'])) {
            $customerDetail = $customer_info_array['@attributes']['Text'].','.str_replace("Email:", "",
            $customer_info_array['ContactPerson'][7]['@attributes']['ContactType']).','.str_replace("Phone:", "",
            $customer_info_array['ContactPerson'][0]['@attributes']['ContactType']);
        } else {
            $customerDetail = $customer_info_array['@attributes']['Text'].','.'NA'.','.str_replace("Phone:", "",
            $customer_info_array['ContactPerson'][0]['@attributes']['ContactType']);
        }
        ...
        $price = array();
        for ($i=0;$i<$rooms_qty;$i++) { $price[]=($amount/$rooms_qty); } $room_price=implode(',', $price);
            $currency=$hotelReservation_data['RoomStays']['RoomStay']["RoomRates"]["RoomRate"]["Rates"]["Rate"]["Base"]["@attributes"]["CurrencyCode"];
            //Payment status $payment_status="NA" ;
            //$hotelReservation_data['RoomStays']['RoomStay']['TPA_Extensions']['TotalPrepayAmount']> 0 ?
            // 'Paid('.$hotelReservation_data['RoomStays']['RoomStay']['TPA_Extensions']['TotalPrepayAmount'].')':'Pay at
            //hotel';
            ...
            for ($i=0;$i<$rooms_qty;$i++) {
             $price[]=($amount/$rooms_qty); 
                ...
                $room_price=implode(',', $price); //Payment status
                $payment_status="NA" ; //
                $hotelReservation_data['RoomStays']['RoomStay']['TPA_Extensions']['TotalPrepayAmount']> 0 ?
                'Paid('.$hotelReservation_data['RoomStays']['RoomStay']['TPA_Extensions']['TotalPrepayAmount'].')':'Pay at
                hotel';
                ...
                $push_by = "Travelguru";
                    if ($db_status) {
                        $this->instantbucket->bucketEngineUpdate($booking_status, $push_by, $ota_hotel_details,
                        $ota_booking_tabel_id);//this function is used for booking bucket data updation
                    }
                }
    ...
    public function cancelBooking($hotelReservation_data, $hotel_Code)
    {
        ...
        if ($otaBookingUpdateModel) {
            ...
            $db_status = $bookinginfo['db_status'];
            if ($db_status) {
                $this->instantbucket->bucketEngineUpdate($booking_status, $push_by, $ota_details_data,
                $bookinginfo['ota_booking_tabel_id']);//this function is used for booking bucket data updation
            }
        } else {
