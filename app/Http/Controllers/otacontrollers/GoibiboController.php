 ...
 * @date-23/01/2019
 */
class GoibiboController extends Controller
{
        ...
        if (isset($headers['Authorization']))
        {
          ...
          if($key=='3CBAB7FD66D4E8FDA39F9398595C7')
          {
            if($postdata!='')
            {
              ...
              $amount         = number_format((float)$array_data['Booking']['Price'], 2, '.', '');
              $currency       = $array_data['Booking']['Currency']['@attributes']['short'];
              $channel_name   = $array_data['Booking']["BookingVendorName"];
              $tax_amount     = number_format((float)$array_data['Booking']["PriceDetails"]["GST"], 2, '.', '');
              $price = array();
