...
use App\Jobs\Expedia;

/**
* Expedia Controller used for booking from Expedia .
* @auther Ranjit (modified by)
* @date-23/01/2019
*/
class ExpediaTestController extends Controller
{
    ...
    public function actionIndex(Request $request)
    {
        ...
        if($postdata!='')
        {
                ...
                $ota_details_data               = DB::table('cm_ota_details')->select('*')
                    ...
                    {   //New Booking
                        ...
                        {
                            $updateXML = DB::table('expedia_xml')->insert(['xml'=>$postdata]);
                        ...
                        $responsedata_reciver_ResID_Value   = rand();
    ...
    public function expediaBooking(Request $request){
        $ota_details_model     = new CmOtaDetails();
        $OtaAllAutoPushModel   = new CmOtaAllAutoPush();
        $request_ip            = $_SERVER['REMOTE_ADDR'];
        $headers               = apache_request_headers();
        $getData               = DB::table('expedia_xml')->select('xml','id')->where('is_update',0)->first();
        $update                = DB::table('expedia_xml')->where('id',$getData->id)->update(['is_update'=>2]);
        $postdata              = $getData->xml;
        ...
        if($postdata!='')
        {
            ...
            {   //New Booking
                if($requestType == 'OTA_HotelResNotifRQ')
                {
                    ...
                    $booking_date 					= $booking_root->CreateDateTime;
                    $UniqueID 						= $root->UniqueID->attributes()->ID;
