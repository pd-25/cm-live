...
/**
 * This controller is used for Via single,bulk,sync and block of inv and rate
 * @auther ranjit
 * created date 07/03/19.
 * modification due to display problem @ 12/03/19 by ranjit(new model added)
 */
class ViaInvRateController extends Controller
{
            ...
            foreach($inventory['inv'] as $inv)
            {
                ...
                if($room_code)
                {
                    ...
                    if($inv['block_status']==0)
                    {
                        if($room_qtys >= 0)
                        {
                            $url= $commonUrl.'newWebserviceAPI?actionId=cm_updateroominventory&typeId=hotel&source='.$source.'&auth='.$auth.'&requestData={"hotelId":'.$ota_hotel_code.',"roomId":'.$room_code.',"startDate":"'.$startDate.'","endDate":"'.$endDate.'","available":'.$room_qtys.',"stopSell":"false"}';
                        }
                    }
                    else
                    {
                        $url = $commonUrl.'newWebserviceAPI?actionId=cm_stopsellbyroomid&typeId=hotel&source='.$source.'&auth='.$auth.'&requestData={"hotelId":'.$ota_hotel_code.',"roomId":'.$room_code.',"startDate":"'.$startDate.'","endDate":"'.$endDate.'","stopSell":"true"}';
                    }
                    ...
                    if(isset($array_data['Success'])){
                        ...
                        $rlt=array('status'=>1,'ota_name'=>'via.com','response_msg'=>$result);

                        }else{
                            DB::table('log_table')->where('id', $logModel->id)
                            ->update(['status' => 0,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                            $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>$result);
                ...
                else
                {
                    ...
                    $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Roomtype should be sync");
                }
            }
            if(sizeof($rlt)==0){
                $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Roomtype should be mapped");
            }
        return $rlt;
    }
            ...
            if($room_code)
            {
                ...
                $array_data = (array) json_decode($result);
                    if(isset($array_data['Success'])){
                            ...
                            ->update(['status' => 1,'request_msg'=>$url,'response_msg'=>$result]);
                        $rlt=array('status' => 1,'ota_name'=>'via.com','response'=> ' blocked successfully');
                        }
                    else{
                            DB::table('log_table')->where('id', $blocklog->id)
                            ->update(['status' => 0,'request_msg'=>$url,'response_msg'=>$result]);
                        $rlt=array('status' => 0,'ota_name'=>'via.com','response'=> $result);
                    }
            }
            else{
                $log_data                 		= [
                ...
                $blocklog->fill($log_data)->save();
                $rlt=array('status' => 0,'ota_name'=>'via.com','response'=> 'Roomtype should be Sync');
            }
        }
        if(sizeof($rlt)==0){
            $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Roomtype should be mapped");
    ...
    public function rateSyncUpdate($rate_bucket_data,$rates_data,$auth_parameter,$commonUrl,$from_date,$to_date)
    {
        ...
        foreach($rates_data as $rate)
        {
            foreach($rates_data['rates'] as $rates)
            {
                ...
                $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($multiple_days ,"Via.com");
                ...
                if($rate_code)
                {
                    ...
                    curl_close($ch);
                        ...
                        $logModel->fill($log_data)->save();
                        $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Rateplan should be sync");
                    }
            }
        }
        if(sizeof($rlt)==0){
            $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Rateplan should be mapped");
        }
        return $rlt;
    }
    public function singleRateUpdate($rate_bucket_data,$rates_data,$auth_parameter,$commonUrl)
    {
        ...
        foreach($rates_data as $rate)
        {
            ...
            foreach($rate['rates'] as $rates)
            {
                ...
                $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($rates['multiple_days'],"Via.com");
                $currency=$this->getdata_curlreq->getCurrency($hotel_id);
                $extra_adult_price			 = 0;
                $extra_child_price			 = 0;
                $rateplan_bar_price          = $rates['bar_price'];
    ...
    public function bulkRateUpdate($rate_bucket_data,$data,$auth_parameter,$commonUrl)
    {
        ...
        $currency=$this->getdata_curlreq->getCurrency($hotel_id);
        $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($data['multiple_days'],"Via.com");
        ...
        if(isset($result[0])){
            ...
            $rlt=array('status'=>0,'ota_name'=>'via.com','response_msg'=>"Room type or Rateplan should be sync");
            return $rlt;
        }
    ...
    public function blockRateUpdate($rate_bucket_data,$rooms,$data,$auth_parameter,$commonUrl)
    {
        ...
        $max_adult=$roomTypeModel->select('max_people')->where('room_type_id',$data['room_type_id'])->first()->max_people;
            ...
            $cond = array('hotel_id'=>$data['hotel_id'],'room_type_id'=>$rooms,'rate_plan_id'=>$data['rate_plan_id'],'channel'=>'Via.com');
            $getRateDetails = OtaRatePlan::select('*')
                ...
                if($success_flag){
                    ...
                    if($rate_code){
                        ...
                        if($extra_adult_price!=0 && $extra_child_price!=0)
                        {
                            $string='{"hotelId":'.$ota_hotel_code.',"ratePlanCode":'.$rate_code.',"startDate":"'.$startDate.'","endDate":"'.$endDate.'","extraAdult":0,"extraChild":0,"sellRate":0,"currency":"INR","stopSell":"false","days":"'.$rateplan_multiple_days_data.'"}';
                        }
                        else{
                            $string='{"hotelId":'.$ota_hotel_code.',"ratePlanCode":'.$rate_code.',"startDate":"'.$startDate.'","endDate":"'.$endDate.'","sellRate":0,"currency":"INR","stopSell":"false","days":"'.$rateplan_multiple_days_data.'"}';
                        }
                        if ($rate_type == "B2C"){
                            $url    = $commonUrl.'newWebserviceAPI?actionId=cm_updateroomrates&typeId=hotel&source='.$source.'&auth='.$auth.'&requestData='.$string;
                        }else{
                        ...
                        curl_close($ch);
                            $array_data = (array) json_decode($result);
                              if(isset($array_data['Success'])){
                                DB::table('log_table')->where('id', $logModel->id)
                                ->update(['status' => 1,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                                $rlt=array('status'=>1,'ota_name'=>'via.com','response_msg'=>"update successfully");
                            }
                            else{
                                DB::table('log_table')->where('id', $logModel->id)
                                ->update(['status' => 0,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
