...
/**
 * This controller is used for Paytm single,bulk,sync and block of inv and rate
 * @auther ranjit
 * created date 02/03/19.
 * modification due to display problem @ 12/03/19 by ranjit(new model added)
 */
class PaytmInvRateController extends Controller
{
        ...
        $rlt                            = array();
            ...
            foreach($inventory['inv'] as $inv)
            {
                ...
                if($room_code)
                {
                    ...
                    $log_request_msg = $paytm_xml;
                    $logModel->fill($log_data)->save();//saving pre logdata

                    $result=$this->getdata_curlreq->cUrlCall($url,$headers,$paytm_xml);//for curl call
                    ...
                    if(isset($array_data["status"])){
                        if($array_data["status"] == "success"){
                            ...
                            $rlt=array('status' => 1,'ota_name'=>'paytm','response_msg'=> 'sync successfully');
                            }
                            else{
                                DB::table('log_table')->where('id', $logModel->id)
                                ->update(['status' => 0,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                                $rlt=array('status' => 0,'ota_name'=>'paytm','response_msg'=> $result);
                            }
                        }
                        else{
                            DB::table('log_table')->where('id', $logModel->id)
                    ...
                    $log_data                 	= [
                        ...
                        $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Roomtype should be sync");
                    }
            }
            if(sizeof($rlt)==0){
                $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Roomtype should be mapped");
            }
        return $rlt;
    }
        ...
        if($success_flag)
        {
            ...
            if(isset($array_data["status"])){
                if($array_data["status"] == "success"){
                    ...
                    ->update(['status' => 1,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                    $rlt=array('status' => 1,'ota_name'=>'paytm','response_msg'=> 'blocked successfully');
                ...
                $blocklog->fill($log_data)->save();
                $rlt=array('status' => 0,'ota_name'=>'paytm','response'=> 'Roomtype should be Sync');
            }
        }
        if(sizeof($rlt)==0){
            $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Roomtype should be mapped");
                        ...
                        if($room_qtys >= 0)
                        {
                            ...
                            if(isset($array_data["status"])){
                                if($array_data["status"] == "success"){
                                    DB::table('log_table')->where('id', $logModel->id)
                                    ->update(['status' => 1,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                                    $rlt=array('status' => 1,'ota_name'=>'paytm','response_msg'=> 'update successfully');
    ...
    public function rateSyncUpdate($rate_bucket_data,$rates_data,$auth_parameter,$commonUrl,$from_date,$to_date)
    {
        ...
        $count_rate                      = 0;
            ...
            foreach($rates_data['rates'] as $rates)
            {
                ...
                $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($multiple_days,"Paytm");
                ...
                if($rate_codes)
                {
                    ...
                    $xml='{
                        ...
                        $paytm_rate_xml =$xml;
                        $log_request_msg = $paytm_rate_xml;
                        $logModel->fill($log_data)->save();
                        $result=$this->getdata_curlreq->cUrlCall($url,$headers,$paytm_rate_xml);//for curl call
                        $array_data =json_decode($result,true);
                        if($array_data["status"] == "success"){
                          DB::table('rate_update_logs')->where('id', $logModel->id)
                          ->update(['status' => 1,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                          $rlt=array('status'=>1,'ota_name'=>'paytm','response_msg'=>$result);
                      }
                      else{
                          DB::table('rate_update_logs')->where('id', $logModel->id)
                          ->update(['status' => 0,'request_msg'=>$log_request_msg,'response_msg'=>$result]);
                    ...
                    $logModel->fill($log_data)->save();
                    $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Rateplan should be sync");
                }
            }
        if(sizeof($rlt)==0){
            $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Rateplan should be mapped");
        }
        return $rlt;
    }
    public function singleRateUpdate($rate_bucket_data,$rates_data,$auth_parameter,$commonUrl)
    {
        ...
        foreach($rates_data as $rate)
        {
            ...
            foreach($rate['rates'] as $rates)
            {
                ...
                $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($rates['multiple_days'],"Paytm");
                $currency=$this->getdata_curlreq->getCurrency($hotel_id);
                $extra_adult_price			 = 0;
                $extra_child_price			 = 0;
                $occupency=$this->getdata_curlreq->decideOccupencyPrice($max_adult,$rates['bar_price'],$rates['multiple_occupancy']);
    ...
    public function bulkRateUpdate($rate_bucket_data,$data,$auth_parameter,$commonUrl)
    {
        ...
        $currency=$this->getdata_curlreq->getCurrency($hotel_id);
        $rateplan_multiple_days_data = $this->getdata_curlreq->getDaysUpdate($data['multiple_days'],"Paytm");
        ...
        $rate_code                   = $result[0]['ota_rate_plan_id'];
            ...
            $rlt=array('status'=>0,'ota_name'=>'paytm','response_msg'=>"Room type or Rateplan should be sync");
            return $rlt;
        }
    ...
    public function blockRateUpdate($rate_bucket_data,$rooms,$data,$auth_parameter,$commonUrl)
    {
        ...
        $max_adult=$roomTypeModel->select('max_people')->where('room_type_id',$data['room_type_id'])->first()->max_people;
            ...
            $cond = array('hotel_id'=>$data['hotel_id'],'room_type_id'=>$rooms,'rate_plan_id'=>$data['rate_plan_id'],'channel'=>'Paytm');
