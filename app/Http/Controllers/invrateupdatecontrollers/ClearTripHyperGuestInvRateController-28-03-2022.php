        ...
        foreach($inventory['inv'] as $inv)
        {
            ...
            if($room_code && $ratePlanTypeSynchronizeData)
            {
                ...
                if($inv['block_status']==0)
                {
                    ...
                    $xml_data .= '<AvailStatusMessage BookingLimit="10"><StatusApplicationControl Start="'.$startDate.'" End="'.$endDate.'" InvTypeCode="'.$room_code.'" RatePlanCode="" /><RestrictionStatus Status="Open" /></AvailStatusMessage>';
                    }
                }else{
                    $xml_data .= '<AvailStatusMessage BookingLimit="10"><StatusApplicationControl Start="'.$startDate.'" End="'.$endDate.'" InvTypeCode="'.$room_code.'" RatePlanCode="" /><RestrictionStatus Status="Close" /></AvailStatusMessage>';
                }
            }else{
                $flag=0;
            }
            ...
            if($success_flag)
            {
                ...
                ->get();
                 $xml_data = '<OTA_HotelAvailNotifRQ xmlns="http://www.opentravel.org/OTA/2003/05" Version="1.0" EchoToken="1234">
                                  <AvailStatusMessages HotelCode="H1234">
                                    <AvailStatusMessage BookingLimit="10">';// this consider as room qty
                foreach($ratePlanTypeSynchronizeData as $rtsd)
                {
                    if($room_code && $rtsd )
                    {
                        ...
                        $flag = 1;
                        $xml_data .= '<StatusApplicationControl Start="'.$startDate.'" End="'.$endDate.'" InvTypeCode="'.$rateplan_code.'" RatePlanCode="'.$rateplan_code.'" /><RestrictionStatus Status="Close" />';
                        $xml_data.=$xml;
                    }else{
                        $flag = 0;
                    }
    ...
    public function bulkInvUpdate($bucket_data,$data,$auth_parameter,$commonUrl)
    {
        ...
        if($success_flag)
        {
            ...
            if($room_code && $ratePlanTypeSynchronizeData)
            {
                ...
                {
                    $rateplan_code = $rtsd->ota_rate_plan_id;            
                    $xml .= '<AvailStatusMessage BookingLimit="10">//inventory should be specified
                              <StatusApplicationControl Start="'.$startDate.'" End="'.$endDate.'" InvTypeCode="'.$room_code.'" RatePlanCode="'.$rateplan_code.'" />
                              <RestrictionStatus Status="Open" />
                                <LengthsOfStay>
                                    <LengthOfStay MinMaxMessageType="SetMinLOS" Time="'.$inventory_los.'"/>
