...
class BookingjiniPMSController extends Controller
{
    ...
    */
    ...
    public function pmsRateUpdate(Request $request){
      ...
      $xml_data = '<OTA_HotelRateAmountNotifRQ xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance"  xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema" xmlns="http://www.bookingjini.com/OTA/2017/01"  EchoToken="'.$key.'" TimeStamp="2019-07-04T00:00:00.00+05:30" Version="0">
                      <RateAmountMessages HotelCode="'.$data["hotel_id"].'">
                          <RateAmountMessage>
                              ...
                              <Rates>
                                  <Rate Start="'.$data["date_from"].'" End="'.$data["date_to"].'">
                                      <BaseByGuestAmts>
                                         <BaseByGuestAmt BarPrice="'.$data["price"].'" NumberOfGuests="2" />
      ...
      $callToUpdate = $this->rateUpdateToBookingjini($xml_data);
       if($callToUpdate){
            $callToUpdate = array("status"=>'success',"code"=>200,"message"=>'Rates updated sucessfully');
            return response()->json($callToUpdate);
        }
    }
    ...
    public function inventoryUpdateToBookingjini($xml_data,$blk_status,$pms){
      ...
      if($count<=0) {
       ...
       else if($hotel_code!='')
       {
         ...
         if($pms == 'IDS'){
            $roomDetails=PmsRoom::where([['hotel_id',$hotel_code],
            ['ids_hotel_code',$hotel_code],['room_type_id',$ota_room_type]])->first();
         }
         if($pms == 'KTDC'){
            $roomDetails=KtdcRoom::where([['hotel_id',$hotel_code],
            ['ktdc_hotel_code',$hotel_code],['room_type_id',$ota_room_type]])->first();
         }
         ...
         if(strtotime($date_to)>=strtotime($date_from))
         {
             ...
             if($pms == 'IDS'){
               ...
               $updateInventory =$idsInvPush->fill($inventory)->save();
                ...
                $ktdcInvPush= new KtdcInvPush();
                $updateInventory =DB::insert("insert ignore into ktdc_inventory_push( hotel_id,room_type_id,date_from,date_to,no_of_rooms,ktdc_id,ktdc_name,los,block_status,client_ip,user_id) values ($hotel_code,$ota_room_type,'$date_from','$date_to',$no_of_rooms,$pms_id,'$pms_name',$los,$block_status,'$ip',0)");
                if($updateInventory){
                    $return_array = '<OTA_HotelInventoryDetailsInfoRS xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance"
                    xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema"  xmlns="http://www.bookingjini.com/OTA/2017/01">
                    <Success>Inventory update sucessfully</Success> </OTA_HotelInventoryDetailsInfoRS>';
         ...
         if(!isset($array_data['RateAmountMessages']['RateAmountMessage']['StatusApplicationControl']) && sizeof($array_data['RateAmountMessages']['RateAmountMessage'] > 0)){
           ...
           foreach($rates_arr as $rate){
            ...
            foreach($rate['Rates']['Rate']['BaseByGuestAmts']['BaseByGuestAmt'] as $key => $occupancy){
                if(isset($occupancy["@attributes"]["BarPrice"])){
                    $data['bar_price'] = $occupancy["@attributes"]["BarPrice"];
                }
                if(isset($occupancy["@attributes"]["MultipleOccupancy"])){
                    $occupancy_data = explode(",",$occupancy["@attributes"]["MultipleOccupancy"]);
                    $data['multiple_occupancy'] = $occupancy_data;
                }
            }
    ...
    public function pmsBookings(Request $request){
      ...
      if($header!='' && $data['hotel_id']!=''){
          ...
          $single_booking = isset($data['single_booking'])?$data['single_booking']:0;
          ...
          if(in_array($hotel_id, $hotels)){
              ...
              if($single_booking == 1){
                $BookingDetails = $this->PmsComponents->searchOneBookings($hotel_id, $last_booking_id, $booking_date_i);
              }
              else{
              ...
              if($single_booking == 1){
                $OTABookingDetails = $this->PmsComponents->searchOneOTABookings($hotel_id, $booking_date_i,$last_booking_id);
            }
              else{
    ...
    public function ktdcInventoryUpdate(Request $request){
        $ktdc_inv_data = $request->getcontent();
        $ktdc_inv_data = str_replace('&',"AND",$ktdc_inv_data);
        $ktdc_info = json_decode(json_encode(simplexml_load_string($ktdc_inv_data)),true);
        $key = '2569e728505192f53a1f265f7f98785e';
        $xml = '<OTA_HotelInvCountNotifRQ xmlns:xsi="http://www.bookingjini.com/2017/XMLSchema-instance"  xmlns:xsd="http://www.bookingjini.com/2017/XMLSchema" xmlns="http://www.bookingjini.com/OTA/2017/01"  EchoToken="'.$key.'" TimeStamp="2019-07-04T00:00:00.00+05:30" Version="0">';
        if(isset($ktdc_info["Property"][0])){
            foreach($ktdc_info["Property"] as $property){
                $ktdc_hotel_code = $property["@attributes"]["Id"];
                $get_hotel_id = KtdcRoom::select('hotel_id')
                                ->where('ktdc_hotel_code',$ktdc_hotel_code)
                                ->first();
                if($get_hotel_id){
                    $hotel_id  = $get_hotel_id->hotel_id;
                    $xml.='<Inventories HotelCode="'.$hotel_id.'">';
                    foreach($property["RoomType"] as $rooms){
                        $ktdc_room_code = $rooms["@attributes"]["ID"];
                        $get_room_id = KtdcRoom::select('room_type_id')
                                        ->where('ktdc_hotel_code',$ktdc_hotel_code)
                                        ->where('ktdc_room_type_code',$ktdc_room_code)
                                        ->first();
                        if($get_hotel_id){
                            $hotel_id  = $get_hotel_id->hotel_id;
                            $xml.='<Inventories HotelCode="'.$hotel_id.'">';
                            if(isset($ktdc_info["Property"]["RoomType"][0])){
                                foreach($ktdc_info["Property"]["RoomType"] as $rooms){
                                    $ktdc_room_code = $rooms["@attributes"]["ID"];
                                    $get_room_id = KtdcRoom::select('room_type_id')
                                                    ->where('ktdc_hotel_code',$ktdc_hotel_code)
                                                    ->where('ktdc_room_type_code',$ktdc_room_code)
                                                    ->first();
                                    if($get_room_id){
                                        $room_id  =  $get_room_id->room_type_id;
                                        foreach($rooms["Availability"] as $availability){
                                            $date_details = isset($availability["@attributes"]["Date"])?$availability["@attributes"]["Date"]:$availability["Date"];
                                            $date_dlt = str_replace('/','-',$date_details);
                                            $date_info = date('Y-m-d',strtotime($date_dlt));
                                            $room_info = isset($availability["@attributes"]["Rooms"])?$availability["@attributes"]["Rooms"]:$availability["Rooms"];
                                            $no_of_room = (int)$room_info;
                                            $xml.='<Inventory>
                                                        <StatusApplicationControl Start="'.$date_info.'" End="'.$date_info.'" InvTypeCode="'.$room_id.'" />
                                                        <InvCounts>
                                                        <InvCount Count="'.$no_of_room.'"/>
                                                        </InvCounts>
                                                    </Inventory>';
                                        }
                                    }
                                        ...
                                        <InventoryUpdateStatus Message="Room type is not mapped with Bookingjini of property id'.$ktdc_hotel_code.'and roomtype id '.$ktdc_room_code.'"
                                        Status="Failour"/>';
                                        return $response;
                                    }
                                }
                                    ...
                                    $ktdc_room_code = $ktdc_info["Property"]["RoomType"]["@attributes"]["ID"];
                                    $get_room_id = KtdcRoom::select('room_type_id')
                                                    ->where('ktdc_hotel_code',$ktdc_hotel_code)
                                                    ->where('ktdc_room_type_code',$ktdc_room_code)
                                                    ->first();
                                    if($get_room_id){
                                        $room_id  =  $get_room_id->room_type_id;
                                        if(isset($ktdc_info["Property"]["RoomType"]["Availability"][0])){
                                            foreach($ktdc_info["Property"]["RoomType"]["Availability"] as $availability){
                                                $date_details = $availability["@attributes"]["Date"];
                                                $date_dlt = str_replace('/','-',$date_details);
                                                $date_info = date('Y-m-d',strtotime($date_dlt));
                                                $room_info = $availability["@attributes"]["Rooms"];
                                                $no_of_room = (int)$room_info;
                                                $xml.='<Inventory>
                                                            <StatusApplicationControl Start="'.$date_info.'" End="'.$date_info.'" InvTypeCode="'.$room_id.'" />
                                                            <InvCounts>
                                            ...
                                            $date_details = $ktdc_info["Property"]["RoomType"]["Availability"]["@attributes"]["Date"];
                                            $date_dlt = str_replace('/','-',$date_details);
                                            $date_info = date('Y-m-d',strtotime($date_dlt));
                                            $room_info = $ktdc_info["Property"]["RoomType"]["Availability"]["@attributes"]["Rooms"];
                                            $no_of_room = (int)$room_info;
                                            $xml.='<Inventory>
                                                        <StatusApplicationControl Start="'.$date_info.'" End="'.$date_info.'" InvTypeCode="'.$room_id.'" />
                                                        <InvCounts>
                    ...
                    $response = '<?xml version="1.0" encoding="utf-8"?>
                    <InventoryUpdateStatus Message="Property is not mapped with Bookingjini property id: '.$ktdc_hotel_code.'"
                    Status="Failour"/>';
                    return $response;
                }
            }
            ...
            $ktdc_hotel_code = $ktdc_info["Property"]["@attributes"]["Id"];
            $get_hotel_id = KtdcRoom::select('hotel_id')
                            ->where('ktdc_hotel_code',$ktdc_hotel_code)
                            ->first();
            if($get_hotel_id){
        ...
        $blk_status = 0;
        $pms = 'KTDC';
        $response = $this->inventoryUpdateToBookingjini($xml,$blk_status,$pms);
        if($response){
            $res_info = json_decode(json_encode(simplexml_load_string($response)),true);
            if(isset($res_info["Error"])){
                $response = '<?xml version="1.0" encoding="utf-8"?>
                <InventoryUpdateStatus Message="'.$res_info["Error"].'"
                Status="Failour" />';
            }
            else{
                $response = '<?xml version="1.0" encoding="utf-8"?>
                <InventoryUpdateStatus Message="'.$res_info["Success"].'"
                Status="Success" />';
            }  
        }
        return $response;
